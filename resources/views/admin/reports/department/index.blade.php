        ...
        <div class="col-md-4">
          <div class="card card-primary m-3">
            <form  method="post">
              @csrf 
              <div class="card-header">
                <h6 class="card-text">Generate Report of Product By Department & Category & SubCategory   </h6>
              </div>
              <div class="card-body">
                <div class="form-group">
                  <label> Select Depertment </label>
                <select name="SearchByDepartment_3" class="form-control">
                  <option></option>
                  @foreach ($departments as $department )
                      <option>{{$department->name}}</option>
                  @endforeach
                </select>
                @if($errors->has('SearchByDepartment_3'))
                  <span class="text-danger">Department Name must be Provided!   {{$errors->first('SearchByDepartment_3')}} </span>
                @endif
                </div>
                <div class="form-group">
                  <label> Select Category  </label>
                ...
                @if($errors->has('SearchByCategory_3'))
                  <span class="text-danger">Category Name must be Provided!   {{$errors->first('SearchByCategory_3')}} </span>
                @endif
                </div>
                <div class="form-group">
                ...
                @if($errors->has('SearchBySubCategory_3'))
                  <span class="text-danger"> Subcategory Name must be Provided! {{$errors->first('SearchBySubCategory_3')}} </span>
                @endif
                </div>
              </div>
              <div class="card-footer">
                <button class="btn btn-success" type="submit" formaction="{{route('report.GenByDepartmentCatSubCatToPDF')}}" ><i class="fas fa-file-pdf"></i>Download PDF</button> 
              </div>
            </form>
          </div>
        </div>
